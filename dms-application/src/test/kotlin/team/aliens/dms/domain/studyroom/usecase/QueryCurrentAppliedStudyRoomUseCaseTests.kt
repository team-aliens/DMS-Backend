...
class QueryCurrentAppliedStudyRoomUseCaseTests {
    ...
    fun `자습실 목록 조회 성공`() {
        ...
        every { queryStudyRoomPort.querySeatApplicationsByStudentId(userId) } returns seatApplicationStub
    ...
    fun `seatApplication이 존재하지 않음`() {
        // given
        every { securityPort.getCurrentUserId() } returns userId
        every { queryStudyRoomPort.querySeatApplicationsByStudentId(userId) } returns null
